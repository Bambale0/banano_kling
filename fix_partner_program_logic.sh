...
# Restore broken common.py from current git index if it contains unterminated partner fragment symptoms.
common = Path("bot/handlers/common.py")
s = common.read_text(encoding="utf-8")

# Patch main menu text to NEUROMIX safely if function exists.
s = re.sub(
...
partner_func = '''async def render_partner_program(target, user_id: int):
    ...
    referral_link = (
        f"https://t.me/{me.username}?start=ref_{referral_code}" if referral_code else "Ссылка появится после активации"
    )
...
# Patch database referral logic.
db_path = Path("bot/database.py")
db = db_path.read_text(encoding="utf-8")

# process_referral defaults and behavior.
db = db.replace("signup_bonus: int = 5", "signup_bonus: int = 25")
# Remove central master language in docstring/comment by replacing function body more robustly.
process_func = '''async def process_referral(
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        logger.info(
            "Referral processed: referred=%s, referrer_id=%s, signup_bonus=%s, inviter_bonus=%s",
            referred_telegram_id,
            referrer["id"],
            signup_bonus,
            inviter_bonus,
...
echo "Final partner program logic patched. Restart bot: ./stop.sh && ./start.sh"
