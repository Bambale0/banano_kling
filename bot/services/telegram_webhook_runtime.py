...
ShutdownHook = Callable[[Bot], Awaitable[None]]
...
_settings_cache: TelegramWebhookSettings | None = None


def load_settings() -> TelegramWebhookSettings:
    global _settings_cache
    if _settings_cache is None:
        _settings_cache = TelegramWebhookSettings(
...
_legacy_on_startup: StartupHook | None = None
_legacy_on_shutdown: ShutdownHook | None = None
_installed = False
...
async def ensure_runtime(bot: Bot, dispatcher: Dispatcher) -> None:
...
async def stop_runtime(*, drain: bool = True) -> None:
    global _queue, _runtime_bot, _runtime_dispatcher
...
async def secure_on_startup(
    ...
    if _legacy_on_startup is None:
        raise RuntimeError("Secure Telegram runtime is not installed")
...
async def secure_on_shutdown(bot: Bot) -> None:
    if _legacy_on_shutdown is None:
        raise RuntimeError("Secure Telegram runtime is not installed")
    await stop_runtime()
...
def install_into(legacy_module: Any) -> None:
    global _legacy_on_startup, _legacy_on_shutdown, _installed
    if _installed:
        return

    _legacy_on_startup = legacy_module.on_startup
    _legacy_on_shutdown = legacy_module.on_shutdown
    legacy_module.handle_telegram_webhook = handle_secure_telegram_webhook
    legacy_module.on_startup = secure_on_startup
    legacy_module.on_shutdown = secure_on_shutdown
    _installed = True


async def reset_runtime_for_tests() -> None:
    global _settings_cache
    await stop_runtime(drain=False)
    async with _local_dedupe_lock:
        _local_dedupe.clear()
