...
Telegram-бот для генерации изображений и видео с использованием передовых AI-моделей (NanoBanana, GPT Image 2, Kling 3.0/2.6, Seedream, Grok). Поддерживает:
- Text-to-Image/Video
...
banano_kling/
├── bot/
│   ├── main.py              # aiohttp сервер + webhook handlers (Telegram, Kling, AI webhooks, T-Bank, Crypto Bot)
...
| **payments.py** | `show_topup_menu()`, `initiate_payment()`, `handle_tbank_webhook()`, `handle_cryptobot_webhook()` |
...
Crypto Bot → /cryptobot/webhook → add_credits() → Notify user
```
...
- Модели: GPT Image 2, NanoBanana(2/Pro), Seedream(4.5/5/Lite/Edit), Z-Image Turbo
