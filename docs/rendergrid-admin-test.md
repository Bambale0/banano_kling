...
Если фото не добавлено, выполняется text-to-image сценарий. Если фото добавлено, его URL автоматически добавляется в запрос как `reference_images`, и пользователь остаётся в том же понятном интерфейсе.
