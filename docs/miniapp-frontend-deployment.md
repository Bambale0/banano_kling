...
Сначала убедитесь, что целевой каталог именно `/srv/banano-miniapp`. Telegram WebView может держать предыдущий HTML/webpack manifest дольше обычного браузера, поэтому hashed assets прошлой версии нельзя удалять одновременно с переключением HTML: иначе закешированная сессия получает `404` на JS и остаётся на экране загрузки.
...
`--delete` для обычного Mini App deploy запрещён. Старые `/_next/static/` chunks очищайте отдельной ротацией только после безопасного окна не менее 24 часов и сохраняйте хотя бы две последние сборки. Перед копированием всегда создавайте полный backup текущего каталога.
...
- все assets из текущего HTML отвечают `200`, а chunks предыдущей версии остаются доступны во время cache-overlap
