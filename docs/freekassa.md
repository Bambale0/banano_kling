...
В ветке `tanyapi` FreeKassa / KASSA — основной рублёвый способ оплаты:

- **карта РФ** — KASSA (`i=36`);
- **СБП** — KASSA (`i=44`);
- **Lava** — резерв для рублёвых карты/СБП и резервных зарубежных способов;
- зарубежная / СНГ — Tribute.

В интерфейсе обычные основные кнопки «Картой» и «СБП» ведут в KASSA. Lava остаётся доступна отдельными явно подписанными резервными кнопками и не перехватывает основной рублёвый flow.
...
### Основной рублёвый flow KASSA

1. Пользователь выбирает пакет.
2. Основные кнопки `💳 Картой · KASSA` и `⚡ СБП · KASSA` сразу выбирают карту РФ (`i=36`) или СБП (`i=44`).
...
Lava не удаляется: рублёвые `Резерв · карта (Lava)` и `Резерв · СБП (Lava)` остаются независимыми callbacks Lava. Зарубежные Lava-карта и PayPal также остаются резервными.
...
3. Открыть пополнение и проверить порядок: `💳 Картой · KASSA` → `⚡ СБП · KASSA` → `🌍 Зарубежная / СНГ` → рублёвый резерв Lava → зарубежный резерв Lava.
4. Проверить Mini App: блок `KASSA · основной способ` должен идти первым, а `Резерв · Lava` — отдельным блоком после него.
...
```text
Карта РФ                 -> KASSA / FreeKassa (i=36)
СБП                       -> KASSA / FreeKassa (i=44)
Резерв · карта (Lava)    -> Lava
Резерв · СБП (Lava)      -> Lava
```
