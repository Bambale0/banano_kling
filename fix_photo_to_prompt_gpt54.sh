...
cat > bot/services/photo_prompt_service.py <<'PY'
...
start = s.index("async def miniapp_photo_to_prompt")
end = s.index("\n\nasync def miniapp_generate_image", start)

new_func = r'''async def miniapp_photo_to_prompt(request: web.Request) -> web.Response:
...
python3 -m py_compile bot/services/photo_prompt_service.py bot/miniapp.py

echo "Photo-to-prompt switched to GPT 5.4 Responses API."
echo "Review changes with: git diff -- bot/services/photo_prompt_service.py bot/miniapp.py"
